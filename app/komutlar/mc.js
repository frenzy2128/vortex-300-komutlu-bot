 ...
 message.channel.send(mcbody);
     ...
     var url = 'http://mcapi.us/server/status?ip=' + args[1] + '&port=' + mcPort;
        ...
        request(url, function (err, response, body) {
            if (err) {
                console.log(err);
                return message.channel.send(new Discord.MessageEmbed().setTitle('Hata!').addField('Sunucu bilgileri alınırken beklenmedik bir hatayla karşılaştık.').setThumbnail("https://cdn.pixabay.com/photo/2013/07/12/19/25/minecraft-154749_960_720.png").setAuthor("XERESSA").setFooter('').setColor("RED").setTimestamp());
            }
            ...
            message.channel.send(new Discord.MessageEmbed().setDescription(status).setThumbnail('https://cdn.pixabay.com/photo/2013/07/12/19/25/minecraft-154749_960_720.png').setColor('RED').setFooter('' + body.motd + ''));
        });
