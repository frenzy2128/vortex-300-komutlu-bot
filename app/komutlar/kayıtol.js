...
if(message.member.roles.cache.has(db.fetch(`kayıtR_${message.guild.id}`)) === true) return message.channel.send(`Zaten sende **` + message.guild.roles.cache.get(db.fetch(`kayıtR_${message.guild.id}`)).name + `** rolü bulunuyor.`)
...
.then(async () => {
   ...
   if (collected.first().content === sifre) { 
     ...
     message.member.roles.add(db.fetch(`kayıtR_${message.guild.id}`)).then(d => {   message.channel.createOverwrite(every, {
  SEND_MESSAGES: false
