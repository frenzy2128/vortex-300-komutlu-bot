...
exports.run = async (bot, message, args) => {
  ...
  var osType = await os.type();
    ...
    cpuStat.usagePercent(function(err, percent, seconds) {
      ...
      setTimeout(() => {
        ...
        .addField(`${dil.stat.ws.shard.pinghead}`, `${dil.stat.ws.shard.ping}`.replace("{ping1}", new Date().getTime() - message.createdTimestamp).replace("{ping2}", bot.ws.ping), true)
   ...
   **${dil.stat.voice}** ${bot.voiceConnections.size.toLocaleString()}
        **${dil.stat.users}**  ${bot.guilds.cache.reduce((a, b) => a + b.memberCount, 0).toLocaleString()}
        **${dil.stat.guilds}** ${bot.guilds.cache.size.toLocaleString()}
