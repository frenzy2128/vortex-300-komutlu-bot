...
if (mesaj.match(x)) return message.reply('Tek zeki sensin zaten!');
  ...
  if (para < mesaj) {
    ...
    message.channel.send(`${client.emojis.cache.get(client.emojiler.paraGitmedi)} Yeterince paran bulunmuyor, sende olan para:  ${bulunanP === null  ? "0" : `${bulunanP}`}`)
} else if  (para > mesaj) {

  const embed = new Discord.MessageEmbed()
  .setColor('RANDOM')
  .setDescription(`${client.emojis.cache.get(client.emojiler.paraGitti)} ${user} Adlı kullanıcıya para yollandı, yollanılan miktar: ${mesaj}`)
