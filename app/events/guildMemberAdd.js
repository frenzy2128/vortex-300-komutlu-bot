...
member.setNickname(`${tag} ${member.user.username}`)
  ...
  if (db.has(`sayac_${member.guild.id}`) === true) {
    ...
    const embed = new Discord.MessageEmbed()
     .setColor('RANDOM')
     .setDescription(`<a:yesil:690267180801065251> Gold üye belirdi. \`${member.user.tag}\`, Ayağa kalkın \`${db.fetch(`sayac_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.members.cache.size}\` Kişi Kaldı <a:yesil:690267180801065251>`)
    ...
    member.guild.channels.cache.get(channel).send(`<a:acik:690266274336342068> <a:giris:617377064898527241> **${member.user.tag}** Sunucuya katıldı! \`${db.fetch(`sayac_${member.guild.id}`)}\` üye olmamıza son \`${db.fetch(`sayac_${member.guild.id}`) - member.guild.members.cache.size}\` üye kaldı!`)
    }};
